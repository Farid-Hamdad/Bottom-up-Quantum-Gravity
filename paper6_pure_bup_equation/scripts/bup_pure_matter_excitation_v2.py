...
# -*- coding: utf-8 -*-
"""
bup_pure_matter_excitation_v2.py

BuP Paper 6 — Test de modes informationnels positifs / négatifs.

Objectif :
    Tester si une excitation positive ou négative du réseau d'intrication
    produit une réponse géométrique de signe différent.

    W1 = W0 + sign * amp * scale * f_i f_j

avec :
    sign = +1 pour mode positive
    sign = -1 pour mode negative

Usage :
    python3 bup_pure_matter_excitation_v2.py \
      --mi-file results_mi_N20_full/MI_N20_lam0.71.csv \
      --k 5 \
      --mode negative \
      --amp 0.15 \
      --sigma 1.0 \
      --output-dir results_pure_matter_antimode/lam071_negative
"""
...
def inject_signed_excitation(W0, f, amp, mode="positive", mask_mode="all", base_mask=None):
    """
    Mode positive :
        deltaW = +amp * scale * f_i f_j

    Mode negative :
        deltaW = -amp * scale * f_i f_j

    On applique clipping W1>=0 et on retourne les statistiques de clipping.
    """
    W0 = clean_matrix(W0)
    ...
    clipping = {
        ...
        "raw_delta_min": float(np.min(raw_delta)),
        "raw_delta_max": float(np.max(raw_delta)),
    ...
    return W1, actual_delta, raw_delta, clipping
...
def make_figures(node_df, edge_df, output_dir, mode):
    ...
    plt.title(f"{mode}: excitation locale vs réponse d_s")
    ...
    if np.sum(good) >= 4 and np.std(x[good]) > 1e-15:
        ...
        plt.plot(xx, coeff[0] * xx + coeff[1], linewidth=2)
    plt.tight_layout()
    ...
    plt.title(f"{mode}: proxy matière vs réponse courbure")
    plt.grid(True, alpha=0.3)
    if np.sum(good) >= 4 and np.std(x[good]) > 1e-15:
        coeff = np.polyfit(x[good], y[good], 1)
        xx = np.linspace(np.min(x[good]), np.max(x[good]), 200)
...
def main():
    ...
    parser.add_argument("--output-dir", type=str, default="results_pure_matter_excitation_v2")
    ...
    print("=" * 92)
    print("BuP Paper 6 — v2 modes informationnels positifs / négatifs")
    print("=" * 92)
    ...
    W1, deltaW, raw_deltaW, clipping = inject_signed_excitation(
        W0,
        f=f,
        amp=args.amp,
        mode=args.mode,
    ...
    for i in range(n):
        dR = np.nan
        if i < len(R1) and np.isfinite(R1[i]) and np.isfinite(R0[i]):
            dR = R1[i] - R0[i]
        ...
        node_rows.append({
            ...
            "delta_R_node": dR,
            "abs_delta_R_node": abs(dR) if np.isfinite(dR) else np.nan,
        })

    node_df = pd.DataFrame(node_rows)
    edge_df = edge_excitation_table(edge0, edge1, deltaW)
    edge_df["mode"] = args.mode

    corr_node_ds = safe_corr(node_df["rho_exc"], node_df["abs_delta_ds"])
    corr_node_R = safe_corr(node_df["T_matter_node"], node_df["abs_delta_R_node"])

    corr_edge_G_abs = safe_corr(edge_df["T_matter_edge"], edge_df["abs_delta_G_proxy"])
    corr_edge_G_signed = safe_corr(edge_df["T_matter_edge"], edge_df["delta_G_proxy"])
    corr_edge_R_abs = safe_corr(edge_df["T_matter_edge"], edge_df["abs_delta_R_edge"])
    ...
    np.savetxt(os.path.join(args.output_dir, "W1_excited.csv"), W1, delimiter=",")
    np.savetxt(os.path.join(args.output_dir, "deltaW_actual.csv"), deltaW, delimiter=",")
    np.savetxt(os.path.join(args.output_dir, "deltaW_raw.csv"), raw_deltaW, delimiter=",")
    ...
    summary = {
        ...
        "correlations": {
            ...
            "edge_Tmatter_vs_abs_delta_G": corr_edge_G_abs,
            "edge_Tmatter_vs_delta_G_signed": corr_edge_G_signed,
            "edge_Tmatter_vs_abs_delta_R": corr_edge_R_abs,
        }
    }
    ...
    make_figures(node_df, edge_df, args.output_dir, args.mode)
    ...
    print(f"clip fraction       : {clipping['clip_fraction']:.6g}")
    print(f"actual delta sum    : {clipping['actual_delta_sum']:.6g}")

    print("-" * 92)
    print("RÉSULTATS — CORRÉLATIONS")
    print("-" * 92)
    print(
    ...
    print(
        ...
        f"Pearson={corr_edge_G_abs['pearson_r']:+.6f} "
        f"p={corr_edge_G_abs['pearson_p']:.3g} "
        f"Spearman={corr_edge_G_abs['spearman_r']:+.6f} "
        f"p={corr_edge_G_abs['spearman_p']:.3g}"
    )
    print(
    ...
    print(
        ...
        f"Pearson={corr_edge_R_abs['pearson_r']:+.6f} "
        f"p={corr_edge_R_abs['pearson_p']:.3g} "
        f"Spearman={corr_edge_R_abs['spearman_r']:+.6f} "
        f"p={corr_edge_R_abs['spearman_p']:.3g}"
    )

    print("-" * 92)
    if np.isfinite(corr_edge_G_abs["spearman_r"]) and corr_edge_G_abs["spearman_r"] > 0:
        verdict = "réponse géométrique positive au proxy matière"
    else:
        verdict = "réponse géométrique négative / conjuguée / non locale"
