...
\section{Density-corrected Laplacian}

To reduce the density bias, define
\begin{equation}
q_i=\sum_j W_{ij}.
\end{equation}

We tested the corrected kernel
\begin{equation}
W_{ij}^{(\alpha)}
=
\frac{W_{ij}}
...
\end{tabular}
\caption{
Density-corrected non-uniform \(S^1\) test. The symmetric correction
\(\alpha=1/2\) gives the best recovery of the pure Laplacian in this
combinatorial construction and nearly flattens the degree distribution.
}
...
\caption{
Density correction reduces the bias of the raw graph Laplacian. In this
controlled test, the symmetric correction \(\alpha=1/2\) gives the best recovery
of the pure Laplace--Beltrami operator.
}
...
--- have different density-drift structures. The normalization must be stated
together with the operator convention.
...
develops a density drift, while the symmetric correction
\begin{equation}
