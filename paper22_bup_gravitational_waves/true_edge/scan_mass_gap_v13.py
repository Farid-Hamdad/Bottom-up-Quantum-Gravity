...
# -*- coding: utf-8 -*-
"""
Paper 22 — BuP GW true edge-Hessian mass-gap scan v13
...
Examples
--------
python3 scan_mass_gap_v13.py \
  --v10-script papers/paper22_bup_gravitational_waves/scripts/bup_gw_true_edge_hessian_v10.py \
  --scan-type combined \
  --output-dir papers/paper22_bup_gravitational_waves/results/mass_gap_v13
"""
...
def make_plots(outdir: Path, rows: List[Dict[str, Any]]):
    ...
    scatter_by_family(a, meff2, "lattice spacing a", r"$m_{eff}^2$", "Mass gap vs lattice spacing", "fig_v13_meff2_vs_a.png")
    scatter_by_family(invL2, meff2, r"$1/L^2$", r"$m_{eff}^2$", "Mass gap vs inverse volume scale", "fig_v13_meff2_vs_invL2.png")
    scatter_by_family(N, meff2, "N nodes", r"$m_{eff}^2$", "Mass gap vs graph size", "fig_v13_meff2_vs_N.png")
    scatter_by_family(N, corr, "N nodes", "mean modal correlation", "Modal response vs graph size", "fig_v13_corr_vs_N.png")
    scatter_by_family(N, c, "N nodes", r"$c_{edge}$", "Edge wave speed vs graph size", "fig_v13_cedge_vs_N.png")
...
def write_markdown(outdir: Path, rows: List[Dict[str, Any]], fits: Dict[str, Any]):
    ...
    (outdir / "results" / "mass_gap_v13_summary.md").write_text("\n".join(lines), encoding="utf-8")
...
def parse_args():
    ...
    p.add_argument("--scan-type", default="combined", help="combined or comma list of fixed-volume,fixed-density,fixed-N-density")
    ...
    p.add_argument("--fixed-density-spacing", type=float, default=0.10)
...
def main():
    ...
    write_csv(outdir / "results" / "mass_gap_v13_summary.csv", rows)
    with open(outdir / "results" / "mass_gap_v13_fits.json", "w", encoding="utf-8") as f:
        json.dump(fits, f, indent=2)
    with open(outdir / "results" / "mass_gap_v13_summary.json", "w", encoding="utf-8") as f:
        json.dump({"rows": rows, "fits": fits}, f, indent=2)
    write_markdown(outdir, rows, fits)
    make_plots(outdir, rows)

    print("\n" + "=" * 100)
    print("Mass-gap v13 scan complete")
    print("=" * 100)
    if pd is not None:
        cols = [
