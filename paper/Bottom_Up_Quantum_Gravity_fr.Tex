...
Ce modèle ne postule aucune symétrie de jauge a priori, mais permet de tester si une structure collective émerge dans le vide.

\subsection{Observables}

À partir de l'état fondamental, nous extrayons :

\begin{itemize}
\item un graphe de rotations effectives,
\item les générateurs locaux associés,
\item un axe dominant éventuel,
\item un paramètre d'ordre angulaire,
\item un ratio singulier mesurant l'alignement.
\end{itemize}

Nous définissons :

\begin{itemize}
\item régime \textbf{U(1)-like} : vecteurs fortement colinéaires,
\item régime \textbf{SO(3)-like} : structure isotrope.
\end{itemize}
...
\item $J_{zz}=2.0$
\item $J_{xy}=0.5$
\item $h_{\mathrm{scale}}=0.1$
\end{itemize}

nous observons :

\begin{itemize}
\item $95\%$ des seeds classés \textbf{U(1)-like},
\item paramètre d'ordre moyen
\[
OP \approx 0.969,
\]
contre
...
\subsection{Dépendance au désordre}

Un scan en intensité de champ montre :
...
\subsection{Dépendance en anisotropie}

Le ratio $J_{zz}/J_{xy}$ contrôle la structure émergente :

\begin{itemize}
\item régime SO(3)-like pour faible anisotropie,
\item transition autour de $J_{zz}/J_{xy} \approx 1$,
\item régime U(1)-like robuste pour $J_{zz}/J_{xy} \ge 2$.
\end{itemize}
...
\begin{itemize}
\item n'est pas isotrope,
\item sélectionne dynamiquement un axe privilégié,
\item présente une structure résiduelle de type U(1)-like.
\end{itemize}

Il ne s'agit pas d'une brisure spontanée stricte au sens des théories de champs, les champs locaux introduisant déjà une anisotropie explicite, mais d'une \textbf{sélection dynamique robuste d'une symétrie effective}.
...
\text{dynamique}
\;\Longrightarrow\;
\text{symétrie effective}.
\]

Il suggère que le vide Bottom-Up doit être compris comme un milieu structuré, porteur d'une organisation interne non triviale.

\subsection{Limites}

\begin{itemize}
\item système de taille finie ($N=8$),
\item classification effective (non group-théorique rigoureuse),
\item absence de dynamique de jauge complète.
\end{itemize}

\subsection{Perspectives}

\begin{itemize}
\item extension à plus grande taille ($N>16$),
\item étude des transitions de phase,
\item connexion avec des champs de jauge effectifs,
\item intégration dans un cadre relativiste émergent.
\end{itemize}
% =====================================================================
...
Ce proxy correspond à l'échelle attendue à partir de la vitesse circulaire dans une approximation de lentille mince, et il est utilisé ici comme diagnostic de premier ordre de la réponse différentielle de lentillage.
...
\begin{itemize}
\item LOW : lentillage plus concentré
\item HIGH : lentillage plus étendu
\end{itemize}

\subsection{Implication physique}

Ce résultat constitue une prédiction falsifiable indépendante des courbes de rotation.

À masse baryonique comparable, la structure radiale du lentillage dépend du régime galactique.

\subsection{Comparaison avec les interprétations basées sur la matière noire}

Le cadre BuP étudié ici diffère conceptuellement des modèles standards à halo sombre.

Dans les approches de type $\Lambda$CDM, l'écart entre courbes baryoniques et dynamique observée est attribué à une composante supplémentaire de masse non lumineuse. Dans cette image, les courbes de rotation comme le lentillage sont principalement contrôlés par la distribution totale de masse effective, typiquement paramétrée par un halo sombre et sa concentration.

À l'inverse, l'interprétation BuP n'attribue pas l'excès gravitationnel à une masse invisible additionnelle, mais à une modification de la géométrie émergente elle-même, encodée phénoménologiquement via une dimension effective et une échelle de transition $r_{\mathrm{trans}}$.

Cela conduit à une logique de prédiction qualitativement différente. Dans BuP, la grandeur centrale n'est pas une masse de halo inférée, mais la réponse dépendante du régime de la géométrie émergente à la structure baryonique. Par conséquent, deux galaxies de contenu baryonique comparable ne sont pas contraintes de présenter la même structure radiale de l'excès de réponse si elles appartiennent à des régimes effectifs distincts.

Les résultats sur le proxy de lentillage renforcent cette distinction. Nous trouvons que la position radiale normalisée du pic de l'excès de déflexion diffère systématiquement entre le régime LOW et le régime HIGH :
\[
...
Cela suggère que, dans BuP, la morphologie radiale de l'excès de lentillage n'est pas déterminée uniquement par l'amplitude totale de la réponse gravitationnelle effective, mais aussi par le régime de géométrie émergente.

Le point discriminant n'est donc pas seulement la possibilité de reproduire certaines courbes de rotation, mais la prédiction de profils de lentillage, de lois d'échelle et de dépendances en régime qui diffèrent systématiquement des attentes issues des modèles à halo sombre.
...
\item cette structure d'échelle se propage aux observables de lentillage, produisant une prédiction falsifiable indépendante des courbes de rotation
\end{itemize}
...
\item le lentillage est ici traité au niveau d'un proxy radial, et non encore d'une métrique optique relativiste complète
\end{itemize}
...
\item identification d'une structure d'échelle pour $r_{\mathrm{trans}}$ et séparation en deux régimes selon la compacité baryonique
\item propagation de cette dichotomie aux observables de lentillage, avec une prédiction falsifiable : $x_{\mathrm{peak}}^{\mathrm{LOW}} < x_{\mathrm{peak}}^{\mathrm{HIGH}}$
\end{itemize}
...
La prédiction de lentillage associée à la dichotomie de régimes offre désormais une voie de test observationnel indépendant, ouvrant la possibilité de confronter le cadre BuP aux données de lentillage fort et faible existantes et futures.

\bigskip

Un résultat complémentaire important concerne la structure interne du vide effectif du modèle Bottom-Up. Nos analyses montrent que ce vide ne se comporte pas comme un milieu isotrope trivial, mais comme un milieu structuré dont les propriétés dépendent de l'anisotropie des interactions et du niveau de désordre local. En particulier, nous observons l'émergence robuste d'une symétrie effective de type U(1), associée à la sélection dynamique d'une direction privilégiée dans l'espace des générateurs locaux.

Cette symétrie n'est pas imposée a priori au niveau microscopique, mais résulte d'un phénomène collectif dans l'état fondamental. Sa robustesse vis-à-vis des choix de reconstruction du graphe (notamment le paramètre $k$ dans les graphes kNN) indique qu'il s'agit d'une propriété intrinsèque de l'état, et non d'un artefact numérique. Ce résultat suggère que, dans le cadre Bottom-Up, les symétries effectives peuvent émerger dynamiquement à partir de la structure d'intrication elle-même.

"Cette émergence est contrôlée par deux paramètres : le rapport d'anisotropie J_zz/J_xy et l'amplitude du désordre local h. Une transition nette est observée autour de J_zz/J_xy ~ 1 et h ~ 0.3, au-delà desquels la symétrie effective redevient SO(3)."

Ainsi, le programme Bottom-Up ne conduit pas seulement à une géométrie et une gravité émergentes, mais également à des structures de symétrie effectives, ouvrant la voie à une compréhension unifiée où géométrie, dynamique et symétrie émergent conjointement de l'organisation informationnelle des états quantiques.
