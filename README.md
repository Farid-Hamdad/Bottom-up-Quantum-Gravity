...
- le **quantique** (théories des champs),
- le **classique** (relativité générale),

mais peine encore à expliquer **pourquoi l’espace-temps existe**,  
et **pourquoi la gravité a une structure thermodynamique et géométrique**.

Ce projet explore une idée simple mais radicale :
...
Tout le reste — temps, espace, dimension, gravité — doit **émerger** de la structure interne de \(|\Psi\rangle\).
...
1. On calcule l’**information mutuelle** \(I(i:j)\) entre paires de qubits.
2. On définit une **distance informationnelle** :
   \[
   d_{ij} = -\log\left(\frac{I(i:j)}{I_{\max}}\right)
   \]
3. On reconstruit une géométrie effective par des méthodes d’embarquement (MDS).
...
- Des états à intrication locale se reconstruisent naturellement en **2D**.
- Des états à intrication non-locale **forcent l’émergence d’une dimension supplémentaire**.

👉 La dimension spatiale n’est **pas postulée**,  
elle est **imposée par l’intrication**.
...
👉 Pas besoin d’être expert en gravité quantique pour suivre.
...
- `appendix/` : figures et analyses avancées
