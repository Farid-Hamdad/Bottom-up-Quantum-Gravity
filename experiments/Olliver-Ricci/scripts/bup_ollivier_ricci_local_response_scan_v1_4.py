...
"""
===============================================================================
BUp Ollivier-Ricci Local Response Batch v1.4
-------------------------------------------------------------------------------
Objectif
--------
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Scan en defect_strength: réponse de la courbure Ollivier-Ricci à une perturbation informationnelle locale."
    )
    ...
    parser.add_argument("--n-seeds", type=int, default=20, help="Nombre de seeds par niveau de défaut")
    ...
    parser.add_argument("--out", type=str, default="results_orc_local_scan_v1_4", help="Dossier de sortie")
    ...
    print("=" * 72)
    print("BUp Ollivier-Ricci Local Response Scan v1.4")
    print("=" * 72)
    ...
    for i, strength in enumerate(strengths, start=1):
        print(f"[{i}/{len(strengths)}] defect_strength={strength:g}")
        strength_log: Dict[str, object] = {"defect_strength": strength, "seeds": []}
        ...
        for k in range(args.n_seeds):
            ...
            try:
                ...
                strength_log["seeds"].append(seed_report)
                ...
                strength_log["seeds"].append({"seed": seed, "error": str(exc)})
        ...
        for row in rows:
            ...
            try:
                ...
                continue
            if np.isfinite(ds) and abs(ds - float(strength)) < 1e-12:
                rows_s.append(row)
        summary_s = aggregate_rows(rows_s, modes)
        strength_log["summary"] = summary_s
        scan_logs.append(strength_log)

        print("   Résumé niveau:")
        for mode in modes:
            stats1 = summary_s[mode]["delta_edge_mean"]
            stats2 = summary_s[mode]["delta_near_minus_far"]
            print(
                f"   - {mode}: "
    ...
    summary_by_strength = aggregate_rows_by_strength(rows, modes, strengths)

    write_rows_csv(rows, outdir / "per_seed_results.csv")
    write_strength_summary_csv(summary_by_strength, outdir / "summary_by_strength_mode.csv")

    response_curve_plot(summary_by_strength, strengths, modes, "delta_edge_mean", outdir / "curve_delta_edge_mean.png")
    response_curve_plot(
        summary_by_strength, strengths, modes, "delta_near_minus_far", outdir / "curve_delta_near_minus_far.png"
    )
    response_curve_plot(
        summary_by_strength, strengths, modes, "common_edge_delta_mean", outdir / "curve_common_edge_delta_mean.png"
    )
    positive_fraction_curve_plot(
        summary_by_strength, strengths, modes, "delta_edge_mean", outdir / "curve_posfrac_delta_edge_mean.png"
    )
    positive_fraction_curve_plot(
        summary_by_strength, strengths, modes, "delta_near_minus_far", outdir / "curve_posfrac_delta_near_minus_far.png"
    )

    for mode in modes:
        boxplot_metric_by_strength(
            rows, mode, "delta_edge_mean", strengths, outdir / f"boxplot_delta_edge_mean_{mode}.png"
        )
        boxplot_metric_by_strength(
            rows, mode, "delta_near_minus_far", strengths, outdir / f"boxplot_delta_near_minus_far_{mode}.png"
        )
    ...
    report = {
        ...
        "summary_by_strength": summary_by_strength,
        "logs": scan_logs,
        "artifacts": {
            "per_seed_results": str(outdir / "per_seed_results.csv"),
            "summary_by_strength_mode": str(outdir / "summary_by_strength_mode.csv"),
            "curve_delta_edge_mean": str(outdir / "curve_delta_edge_mean.png"),
            "curve_delta_near_minus_far": str(outdir / "curve_delta_near_minus_far.png"),
            "curve_common_edge_delta_mean": str(outdir / "curve_common_edge_delta_mean.png"),
            "curve_posfrac_delta_edge_mean": str(outdir / "curve_posfrac_delta_edge_mean.png"),
            "curve_posfrac_delta_near_minus_far": str(outdir / "curve_posfrac_delta_near_minus_far.png"),
        },
    }
    save_json(report, outdir / "report_scan.json")

    print("Résumé agrégé final par niveau:")
    for strength in strengths:
        key = f"{strength:.12g}"
        print(f"- defect_strength={strength:g}")
        for mode in modes:
            stats1 = summary_by_strength[key][mode]["delta_edge_mean"]
            stats2 = summary_by_strength[key][mode]["delta_near_minus_far"]
            print(
                f"   {mode}: "
                f"Δedge mean={stats1['mean']:+.4f} ± {stats1['std']:.4f} "
                f"(pos_frac={stats1['positive_frac']:.2f}, p_sign={stats1['sign_pvalue']}) | "
