...
def main():
    ...
    def g_eff_func(r):
        return g_bary_func(r) * f_d_func(r)
    ...
    out_df.to_csv(outdir / f"{args.galaxy}_optical_metric_v1_1_profile.csv", index=False)

    report_out = {
        "model": "bup_optical_metric_lensing_v1_1",
        "interpretation": "effective acceleration ray-tracing",
        "galaxy": args.galaxy,
        "cosmology": {
            "z_lens": args.z_lens,
        ...
        "diagnostics": {
            ...
            "theta_alpha_bary_max_arcsec": float(theta_arcsec[i_alpha_bary]),
            "R_alpha_bup_max_kpc": float(R_kpc[i_alpha_bup]),
            "theta_alpha_bup_max_arcsec": float(theta_arcsec[i_alpha_bup]),
    ...
    plt.title(f"{args.galaxy} — deflection enhancement v1.1")
    plt.legend()
    plt.tight_layout()
    ...
    print("=" * 72)
    print("BuP OPTICAL METRIC LENSING v1.1 — SUMMARY")
    print("=" * 72)
