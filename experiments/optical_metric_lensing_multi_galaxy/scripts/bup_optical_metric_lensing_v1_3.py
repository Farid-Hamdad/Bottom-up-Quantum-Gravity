...
def robust_ratio_stats(alpha_bup: np.ndarray, alpha_bary: np.ndarray, theta: np.ndarray, frac_threshold: float = 0.1):
    ...
    if np.any(mask):
        return {
            ...
            "theta_window_min_arcsec": float(np.nanmin(theta[mask])),
            "theta_window_max_arcsec": float(np.nanmax(theta[mask])),
        }
    return {
        "mask": mask,
        "ratio": ratio,
        ...
        "median_ratio": np.nan,
        "max_ratio": np.nan,
        "theta_window_min_arcsec": np.nan,
        "theta_window_max_arcsec": np.nan,
    }
...
def main():
    ...
    ap.add_argument("--weight-width", type=float, default=0.5)
    ap.add_argument("--x-center-cap", type=float, default=3.0, help="cap the BuP optical center to keep lensing central")
    ap.add_argument("--r-cut-factor", type=float, default=4.0, help="outer exponential cutoff in units of Rd")
    args = ap.parse_args()
    ...
    x_center = min(x0, args.x_center_cap)
    r_cut_kpc = args.r_cut_factor * Rd_kpc
    w_center_grid = local_weight_x(x_grid, x_center=x_center, width=args.weight_width)
    outer_cut_grid = np.exp(-R_kpc / max(r_cut_kpc, 1e-12))
    w_total_grid = w_center_grid * outer_cut_grid
    ...
    def w_total_func(r):
        ...
        return local_weight_x(x, x_center=x_center, width=args.weight_width) * np.exp(-r / max(r_cut_kpc, 1e-12))
    ...
    out_df = pd.DataFrame({
        ...
        "d_used": d_used,
        "f_d": f_d_grid,
        "w_center": w_center_grid,
        "w_outer_cut": outer_cut_grid,
        "w_total": w_total_grid,
        "v_bary_kms": v_bary,
    ...
    out_df.to_csv(outdir / f"{args.galaxy}_optical_metric_v1_3_profile.csv", index=False)

    report_out = {
        "model": "bup_optical_metric_lensing_v1_3",
        "interpretation": "centralized localized effective acceleration ray-tracing",
        "galaxy": args.galaxy,
        "cosmology": {
            "z_lens": args.z_lens,
        ...
        "bup_profile": {
            ...
            "x_center_cap": args.x_center_cap,
    ...
    plt.plot(theta_arcsec, alpha_bup, label="alpha_BuP centralized")
    plt.plot(theta_arcsec, theta_arcsec, "--", label="alpha=theta")
    plt.xlabel("theta [arcsec]")
    plt.ylabel("alpha(theta) [arcsec]")
    plt.title(f"{args.galaxy} — BuP optical metric v1.3")
    plt.legend()
    plt.tight_layout()
    ...
    plt.title(f"{args.galaxy} — excess deflection v1.3")
    plt.tight_layout()
    ...
    plt.title(f"{args.galaxy} — deflection enhancement v1.3")
    plt.legend()
    plt.tight_layout()
