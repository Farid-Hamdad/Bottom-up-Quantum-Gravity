...
\title{
\textbf{De l'excitation d'intrication à la matière émergente}\\[0.4em]
\large Construction d'une source tensorielle BuP candidate
}
...
contrainte interne \(T_{\rm grad}\), puis une composante de flux \(F\).
...
À partir d'un embedding de la géométrie émergente (obtenu par
multidimensional scaling), on définit les coordonnées locales \(x_i^a\) et la
direction effective normalisée sur l'arête \((i,j)\) :
\[
v_{ij}^a
=
\frac{x_j^a - x_i^a}{d_{ij}^{\rm ent} + \epsilon},
\]
où \(\epsilon\) est un petit régulateur évitant les divisions par zéro.

\subsection{Trace spatiale isotrope}

La trace spatiale, analogue à une pression isotrope, est définie par
\[
T_{aa}(i)
=
\frac12
...
\|v_{ij}\|^2.
\]
Le facteur \(1/2\) est une convention de normalisation, choisie pour que
les coefficients \(\omega, \chi, \psi\) soient de l'ordre de l'unité dans
le protocole normalisé.

\subsection{Contrainte interne (gradient)}

Pour capturer la « rugosité » ou la structure interne de l'excitation,
nous introduisons un terme qui pénalise les variations spatiales de la
densité d'énergie :
\[
T_{\rm grad}(i)
=
...
\right)^2,
\]
où \(A_{ij}\) désigne l'adjacence du graphe \(k\)-NN (éventuellement pondérée
par les poids d'intrication). Ce terme est élevé lorsque la densité d'énergie
change rapidement d'un nœud à l'autre.

\subsection{Flux informationnel}

Le flux, analogue à la composante \(T_{0a}\) du tenseur énergie-impulsion,
est défini par
\[
\vec f(i)
=
...
Dans les tableaux et formules, nous notons symboliquement
\(F(i) \equiv \|T_{0a}(i)\|\).

\section{Sources testées}

Nous testons une hiérarchie de sources, de la plus simple à la plus complète.
...
Le meilleur poids effectif trouvé par scan est :
\[
\omega = -0.5.
\]
...
La réponse géométrique est la variation de courbure nodale :
\[
\delta R_i
=
R_i[W^{(0)} + \delta W]
...
R_i[W^{(0)}],
\]
où \(R_i\) est un proxy de courbure scalaire discret (construit à partir
de la courbure d'Ollivier-Ricci).

Le critère principal est la corrélation de Spearman :
\[
\rho_{\rm Spearman}
\left(
S(i),
...
avec la distance d'intrication au centre. Pour \(\sigma = 0.01\), le
participation ratio (une mesure de localisation) est proche de \(2\),
indiquant une excitation compacte. Pour des valeurs plus grandes de \(\sigma\),
l'excitation devient plus étendue tout en restant localisée géométriquement.
...
\[
T_{00} - \frac12 T_{aa} + \frac12 T_{\rm grad}
\quad\Rightarrow\quad
\rho_{\rm Spearman} \simeq 0.612,
\qquad
p \simeq 0.00413.
\]
...
\[
T_{00} - \frac12 T_{aa} + \frac12 T_{\rm grad} + F
\quad\Rightarrow\quad
\rho_{\rm Spearman} \simeq 0.741,
\qquad
p \simeq 1.84 \times 10^{-4}.
\]
...
\toprule
Source testée & Meilleurs paramètres & Spearman avec \(|\delta R|\) & \(p\)-value \\
\midrule
\(T_{00}\) & \(\sigma = 0.05\) & \(0.195\) & \(0.409\) \\
\(T_{00} - \frac12 T_{aa}\) & \(\sigma = 0.15\) & \(0.526\) & \(0.017\) \\
\(T_{00} - \frac12 T_{aa} + \frac12 T_{\rm grad}\) & \(\sigma = 0.15\) & \(0.612\) & \(0.00413\) \\
\(T_{00} - \frac12 T_{aa} + \frac12 T_{\rm grad} + F\) & \(\sigma = 0.15\) & \(0.741\) & \(1.84 \times 10^{-4}\) \\
\bottomrule
\end{tabular}
\caption{
...
Le flux associé est (version simplifiée d'une divergence discrète) :
\[
\vec f_{\rm ent}(i)
=
-\sum_j A_{ij}
...
Le but est de réduire la norme \(L^2\) de la divergence totale :
\[
\mathcal{D}(\alpha)
=
\left\|
...
Compensation \(\delta d_s\) seul & \(\sigma = 0.15\) & \(0.668\) & faible compensation \\
...
formes d'excitation. Une analyse plus fine du tenseur d'intrication complet
(qui devrait inclure gradients et variations de \(d_s\) de manière covariante)
est laissée à des travaux futurs.
